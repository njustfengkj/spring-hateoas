	...
	static class BadController {
		...
		public Resources<Resource<Employee>> all() {
			...
			Link selfLink = linkTo(methodOn(BadController.class).all()).withSelfRel()
				.withAffordance(afford(methodOn(BadController.class).updateEmployee(null, "0")));
		...
		public Resource<Employee> findOne(@PathVariable String id) {
			...
			Link employeesLink =
				linkTo(methodOn(BadController.class).all()).withRel("employees")
					.withAffordance(afford(methodOn(BadController.class).newEmployee(null)));

			// Return the affordance + a link back to the entire collection resource.
			return new Resource<Employee>(
				EMPLOYEES.get(Integer.parseInt(id)),
				findOneLink
					.addAffordances(employeesLink.getAffordances()),
				employeesLink);
		}
